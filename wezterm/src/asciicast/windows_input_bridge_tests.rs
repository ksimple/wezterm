...
mod tracker_and_run_loop {
    ...
    fn filters_owned_input_modes_and_updates_state() {
        ...
        assert_eq!(output, b"before\x1b[?2004hafter");
    ...
    fn mixed_owned_unowned_reset_filters_only_owned_modes() {
        ...
        assert_eq!(tracker.filter(b"\x1b[?1000;1006;2004h"), b"\x1b[?2004h");
        assert_eq!(
            tracker.filter(b"\x1b[?25;1000;1006;2004l"),
            b"\x1b[?25;2004l"
        );
    ...
    #[test]
    fn oversized_pending_control_strings_are_passed_through() {
        let (_state, mut tracker) = tracker();
        let mut osc = b"\x1b]0;".to_vec();
        osc.resize(MAX_TRACKED_CHILD_OUTPUT_BYTES + 1, b'x');

        assert_eq!(tracker.filter(&osc), osc);
        assert_eq!(tracker.drain_pending(), b"");
    }

    #[test]
    fn oversized_control_string_payload_is_not_reparsed_before_terminator() {
        let (state, mut tracker) = tracker();
        let mut osc = b"\x1b]0;".to_vec();
        osc.resize(MAX_TRACKED_CHILD_OUTPUT_BYTES + 1, b'x');
        ...
        assert_eq!(tracker.filter(&osc), osc);
        let (output, responses) =
            tracker.filter_and_get_responses(b"\x1b[6n\x1b[?9001h\x07after\x1b[5n");

        assert_eq!(output, b"\x1b[6n\x1b[?9001h\x07after");
        assert_eq!(responses, b"\x1b[0n");
        assert!(!state.lock().unwrap().win32_input);
    }

    #[test]
    fn oversized_control_string_handles_split_st_terminator() {
        let (state, mut tracker) = tracker();
        let mut dcs = b"\x1bP".to_vec();
        dcs.resize(MAX_TRACKED_CHILD_OUTPUT_BYTES + 1, b'x');
        dcs.push(0x1b);

        assert_eq!(tracker.filter(&dcs), dcs);
        let (output, responses) = tracker.filter_and_get_responses(b"\\after\x1b[6n");
    ...
    #[test]
    fn oversized_control_string_handles_bel_after_pending_esc() {
        let (state, mut tracker) = tracker();
        let mut osc = b"\x1b]0;".to_vec();
        osc.resize(MAX_TRACKED_CHILD_OUTPUT_BYTES + 1, b'x');
        osc.push(0x1b);
        ...
        assert_eq!(tracker.filter(&osc), osc);
        let (output, responses) = tracker.filter_and_get_responses(b"\x07after\x1b[6n");

        assert_eq!(output, b"\x07after");
        assert_eq!(responses, b"\x1b[1;1R");
        assert!(!state.lock().unwrap().win32_input);
    }

    #[test]
    fn oversized_control_string_handles_c1_st_terminator() {
        let (state, mut tracker) = tracker();
        let mut dcs = b"\x1bP".to_vec();
        dcs.resize(MAX_TRACKED_CHILD_OUTPUT_BYTES + 1, b'x');
        dcs.push(0x9c);
    ...
    #[test]
    fn oversized_control_string_handles_c1_st_after_pending_esc() {
        let (state, mut tracker) = tracker();
        let mut dcs = b"\x1bP".to_vec();
        dcs.resize(MAX_TRACKED_CHILD_OUTPUT_BYTES + 1, b'x');
        dcs.push(0x1b);
        ...
        assert_eq!(tracker.filter(&dcs), dcs);
        let (output, responses) = tracker.filter_and_get_responses(b"\x9cafter\x1b[6n");

        assert_eq!(output, b"\x9cafter");
        assert_eq!(responses, b"\x1b[1;1R");
        assert_eq!(
            state.lock().unwrap().mouse_tracking,
            MouseTrackingMode::None
    ...
    #[test]
    fn oversized_c1_control_string_payload_is_not_reparsed_before_terminator() {
        let (state, mut tracker) = tracker();
        let mut dcs = vec![0x90];
        dcs.resize(MAX_TRACKED_CHILD_OUTPUT_BYTES + 1, b'x');

        assert_eq!(tracker.filter(&dcs), dcs);
        let (output, responses) =
            tracker.filter_and_get_responses(b"\x1b[c\x1b[?1000h\x9cafter\x1b[6n");

        assert_eq!(output, b"\x1b[c\x1b[?1000h\x9cafter");
        assert_eq!(responses, b"\x1b[1;1R");
        assert_eq!(
            state.lock().unwrap().mouse_tracking,
            MouseTrackingMode::None
    ...
    #[test]
    fn oversized_utf8_c1_control_string_payload_is_not_reparsed_before_terminator() {
        let (state, mut tracker) = tracker();
        let mut osc = "\u{9d}0;".as_bytes().to_vec();
        osc.resize(MAX_TRACKED_CHILD_OUTPUT_BYTES + 1, b'x');

        assert_eq!(tracker.filter(&osc), osc);
        let mut continuation = b"\x1b[6n\x1b[?9001h".to_vec();
        continuation.extend_from_slice("\u{9c}".as_bytes());
        continuation.extend_from_slice(b"after\x1b[5n");
        let (output, responses) = tracker.filter_and_get_responses(&continuation);

        let mut expected = b"\x1b[6n\x1b[?9001h".to_vec();
        expected.extend_from_slice("\u{9c}".as_bytes());
    ...
    #[test]
    fn oversized_pending_csi_sequences_are_passed_through() {
        let (_state, mut tracker) = tracker();
        let mut csi = b"\x1b[?".to_vec();
        csi.resize(MAX_TRACKED_CHILD_OUTPUT_BYTES + 1, b'1');

        assert_eq!(tracker.filter(&csi), csi);
        assert_eq!(tracker.drain_pending(), b"");
    ...
    fn filters_terminal_response_queries_from_child_output() {
        ...
        let (output, responses) =
            tracker.filter_and_get_responses(b"a\x1b[c\x1b[>0c\x1b[6n\x1b[?6n\x1b[5nb");

        assert_eq!(output, b"ab");
        assert_eq!(
            responses,
            b"\x1b[?1;0c\x1b[>0;0;0c\x1b[1;1R\x1b[?1;1R\x1b[0n"
        );
    }

    #[test]
    fn utf8_output_continuation_bytes_are_not_c1_control_strings() {
        let (_state, mut tracker) = tracker();
        let mut bytes = "😀中".as_bytes().to_vec();
        bytes.extend_from_slice(b"\x1b[6n");

        let (output, responses) = tracker.filter_and_get_responses(&bytes);

        assert_eq!(output, "😀中".as_bytes());
        assert_eq!(responses, b"\x1b[1;1R");
        assert_eq!(tracker.drain_pending(), b"");
    }

    #[test]
    fn utf8_encoded_c1_csi_queries_and_modes_are_filtered() {
        let (state, mut tracker) = tracker();
        let mut bytes = b"a".to_vec();
        bytes.extend_from_slice("\u{9b}6n".as_bytes());
        bytes.extend_from_slice(b"b");
        bytes.extend_from_slice("\u{9b}?9001h".as_bytes());
    ...
    #[test]
    fn split_utf8_encoded_c1_csi_query_is_filtered() {
        let (_state, mut tracker) = tracker();
        let csi = "\u{9b}6n".as_bytes();

        assert_eq!(tracker.filter(&csi[..1]), b"");
        let (output, responses) = tracker.filter_and_get_responses(&csi[1..]);
    ...
    #[test]
    fn raw_c1_csi_queries_and_modes_are_filtered() {
        let (state, mut tracker) = tracker();

        let (output, responses) = tracker.filter_and_get_responses(b"a\x9b6nb\x9b?9001hc");
    ...
    #[test]
    fn split_raw_c1_csi_query_is_filtered() {
        let (_state, mut tracker) = tracker();

        assert_eq!(tracker.filter(b"\x9b"), b"");
        let (output, responses) = tracker.filter_and_get_responses(b"6n");
    ...
    #[test]
    fn fragmented_utf8_output_is_not_treated_as_c1_control_string() {
        let (_state, mut tracker) = tracker();
        let emoji = "😀".as_bytes();
        let mut output = Vec::new();

        output.extend_from_slice(&tracker.filter(&emoji[..1]));
        let (tail, responses) = tracker.filter_and_get_responses(&emoji[1..]);
    ...
    fn control_strings_preserve_csi_like_payloads_and_st_terminators() {
        for (sequence, split_at) in [
            ...
            (b"\x1b]0;osc \x1b[6n \x1b[?9001h\x9cafter", 9),
            ("\x1b]0;osc \x1b[6n \x1b[?9001h\u{9c}after".as_bytes(), 9),
            (b"\x1bPdcs \x1b[6n \x1b[?9001h\x9cafter", 11),
    ...
    fn control_strings_are_preserved_when_split_at_every_byte() {
        for sequence in [
            ...
            b"\x1b]0;osc \x1b[6n \x1b[?9001h\x9cafter",
            "\x1b]0;osc \x1b[6n \x1b[?9001h\u{9c}after".as_bytes(),
            b"\x1bPpayload \x1b[c \x1b[?1000h\x1b\\after",
            b"\x1bPpayload \x1b[c \x1b[?1000h\x9cafter",
    ...
    #[test]
    fn utf8_encoded_c1_control_strings_are_passed_through_without_state_or_responses() {
        for sequence in [
            "\u{9d}0;osc \x1b[6n \x1b[?9001h\u{9c}after".as_bytes(),
            "\u{90}dcs \x1b[c \x1b[?1000h\u{9c}after".as_bytes(),
            "\u{9f}apc \x1b[5n \x1b[?1002h\u{9c}after".as_bytes(),
            "\u{9e}pm \x1b[6n \x1b[?1003h\u{9c}after".as_bytes(),
            "\u{98}sos \x1b[>0c \x1b[?2004h\u{9c}after".as_bytes(),
        ] {
            let (state, mut tracker) = tracker();
    ...
    #[test]
    fn utf8_encoded_c1_control_strings_are_preserved_across_fragments() {
        for sequence in [
            "\u{9d}0;osc \x1b[6n \x1b[?9001h\u{9c}after".as_bytes(),
            "\u{90}dcs \x1b[c \x1b[?1000h\u{9c}after".as_bytes(),
            "\u{9f}apc \x1b[5n \x1b[?1002h\u{9c}after".as_bytes(),
    ...
    #[test]
    fn c1_control_strings_are_preserved_across_fragments() {
        for sequence in [
            b"\x9d0;osc \x1b[6n \x1b[?9001h\x07after".as_slice(),
            b"\x90dcs \x1b[c \x1b[?1000h\x9cafter",
            b"\x9fapc \x1b[5n \x1b[?1002h\x9cafter",
...
mod keyboard {
    ...
    fn plain_text_preserves_bmp_unicode_and_ignores_unpaired_surrogates() {
        ...
        let low_surrogate = key_record_u16(0, 0, 0xde00, true, 1, 0);

        assert_eq!(encode_records_default(&[e_acute]), "éé".as_bytes());
        assert_eq!(encode_records_default(&[high_surrogate]), b"");
        assert_eq!(
            encode_records_default(&[high_surrogate, low_surrogate]),
            "😀".as_bytes()
        );
        assert_eq!(encode_records_default(&[low_surrogate]), b"");
    }

    #[test]
    fn modified_printable_keys_use_terminal_modifier_encoding() {
        let alt_a = key_record('A' as u16, 0x1e, 'a', true, 1, LEFT_ALT_PRESSED);
        let ctrl_a = key_record('A' as u16, 0x1e, '\x01', true, 1, LEFT_CTRL_PRESSED);

        assert_eq!(encode_records_default(&[alt_a]), b"\x1ba");
        assert_eq!(encode_records_default(&[ctrl_a]), b"\x01");
    }
...
mod batching {
    ...
    fn win_input_batch_preserves_resize_order() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                WinInputBatchMessage::Resize(pty_size(120, 40)),
    ...
    fn win_input_batch_handles_resize_only_and_adjacent_resizes() {
        ...
        assert_eq!(
            ...
            vec![
                WinInputBatchMessage::Resize(pty_size(100, 30)),
                WinInputBatchMessage::Resize(pty_size(120, 40)),
                WinInputBatchMessage::Stdin(b"a".to_vec()),
                WinInputBatchMessage::Resize(pty_size(80, 25)),
            ]
        );
    }
...
mod windows_console_integration {
    ...
    fn require_wezterm_pane_for_record_e2e() -> anyhow::Result<bool> {
        ...
        if std::env::var_os("WEZTERM_RECORD_E2E_SKIP_IF_MISSING").is_some() {
            eprintln!(
                    "skipping: this e2e requires a real WezTerm pane; unset WEZTERM_RECORD_E2E_SKIP_IF_MISSING to fail missing prerequisites"
                );
            return Ok(false);
        }

        anyhow::bail!(
            "wezterm record e2e requires WEZTERM_PANE; set WEZTERM_RECORD_E2E_SKIP_IF_MISSING=1 to skip outside a real pane"
        );
    }
...
while ([DateTime]::UtcNow -lt $deadline) {
  ...
  $n=$task.Result
  if ($n -le 0) { break }
  for ($i=0; $i -lt $n; $i++) { $bytes.Add($buf[$i]) }
  $hexSoFar=($bytes | ForEach-Object { $_.ToString('X2') }) -join ''
  if ($hexSoFar.Contains('610D0A')) { break }
}
if ($bytes.Count -eq 0) {
